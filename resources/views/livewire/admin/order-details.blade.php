        ...
        <table class="table table-striped table-hover">
            ...
            @foreach($orderDetails as $index => $orderDetail)
                <tr>
                    ...
                    <td class="text-center align-middle">{{ $orderDetail->product->name }}</td>
                    <td class="text-center align-middle">{{ number_format($orderDetail->price) }}</td>
                    <td class="text-center align-middle">{{ number_format($orderDetail->discount_price) }}</td>
                    ...
                    <td class="text-center align-middle" wire:click="changeStatus({{ $orderDetail->id }})">
                        ...
                        @elseif($orderDetail->status === \App\Enums\OrderStatus::Processing->value)
                            <span class="badge badge-warning cursor-pointer">در حال پردازش</span>
                        @elseif($orderDetail->status === \App\Enums\OrderStatus::Send->value)
                            <span class="badge badge-info cursor-pointer">ارسال شده</span>
                        @endif
                    </td>
